...
class TeamController extends Controller
{
    ...
    public function store(StoreTeamRequest $request)
    {
        ...
        if ($request->input('profile_avatar', false)) {
            $team->addMedia(storage_path('tmp/uploads/' . basename($request->input('profile_avatar'))))->toMediaCollection('profile_avatar');
        }
    ...
    public function update(UpdateTeamRequest $request, Team $team)
    {
        ...
        if ($request->input('profile_avatar', false)) {
            if (!$team->profile_avatar || $request->input('profile_avatar') !== $team->profile_avatar->file_name) {
                if ($team->profile_avatar) {
                    $team->profile_avatar->delete();
                }
                $team->addMedia(storage_path('tmp/uploads/' . basename($request->input('profile_avatar'))))->toMediaCollection('profile_avatar');
            }
        } elseif ($team->profile_avatar) {
            $team->profile_avatar->delete();
        }
