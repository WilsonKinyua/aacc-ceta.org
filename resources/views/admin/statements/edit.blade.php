...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.statements.update', [$statement->id]) }}"
                enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required"
                        for="statement_document_preview">{{ trans('cruds.statement.fields.statement_document_preview') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('statement_document_preview') ? 'is-invalid' : '' }}"
                        id="statement_document_preview-dropzone">
                    </div>
                ...
                <div class="form-group">
                    <label class="required"
                        for="statement_document">{{ trans('cruds.statement.fields.statement_document') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('statement_document') ? 'is-invalid' : '' }}"
                        id="statement_document-dropzone">
                    </div>
        ...
        Dropzone.options.statementDocumentPreviewDropzone = {
            ...
            init: function() {
                @if (isset($statement) && $statement->statement_document_preview)
                    ...
                    $('form').append('<input type="hidden" name="statement_document_preview" value="' + file
                        .file_name + '">')
                    this.options.maxFiles = this.options.maxFiles - 1
                @endif
            },
