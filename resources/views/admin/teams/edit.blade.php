...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teams.update", [$team->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="profile_avatar">{{ trans('cruds.team.fields.profile_avatar') }}</label>
                <div class="needsclick dropzone {{ $errors->has('profile_avatar') ? 'is-invalid' : '' }}" id="profile_avatar-dropzone">
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.team.fields.profile_avatar_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="bio">{{ trans('cruds.team.fields.bio') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('bio') ? 'is-invalid' : '' }}" name="bio" id="bio">{!! old('bio', $team->bio) !!}</textarea>
                @if($errors->has('bio'))
                    <div class="invalid-feedback">
                        {{ $errors->first('bio') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.team.fields.bio_helper') }}</span>
            </div>
            <div class="form-group">
...
@section('scripts')
<script>
    Dropzone.options.profileAvatarDropzone = {
    ...
    success: function (file, response) {
      $('form').find('input[name="profile_avatar"]').remove()
      $('form').append('<input type="hidden" name="profile_avatar" value="' + response.name + '">')
    },
    ...
    init: function () {
@if(isset($team) && $team->profile_avatar)
      var file = {!! json_encode($team->profile_avatar) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="profile_avatar" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
